...
{
    public partial class LstAjusteInventarioForm : Form
    {
        private ObservableListSource<AjusteInventarioViewModel> ViewModelList = new ObservableListSource<AjusteInventarioViewModel>();

        #region Constructor
        public LstAjusteInventarioForm()
        {
            InitializeComponent();

            ViewModelList = new ObservableListSource<AjusteInventarioViewModel>();
            ajusteInventarioViewModelBindingSource.DataSource = ViewModelList;
        }
        #endregion
        ...
        private void Agregar()
        {
            try
            {
                var frm = new MntAjusteInventarioForm(ViewModelList);
                frm.ShowDialog();
            }
            catch (Exception ex)
            {
        ...
        private void Modificar()
        {
            try
            {
                var viewModel = (AjusteInventarioViewModel)ajusteInventarioViewModelBindingSource.Current;
                var frm = new MntAjusteInventarioForm(viewModel, ViewModelList);
                frm.ShowDialog();
            }
            catch (Exception ex)
            {
        ...
        private void Actualizar()
        {
            ...
            {
                Cursor = Cursors.WaitCursor;
                var list = new ApplicationDbContext().AjusteInventarios.ToList();
                ViewModelList.Clear();
                foreach (var model in list)
                {
                    ViewModelList.Add(new AjusteInventarioViewModel(model));
                }
            }
            catch (Exception ex)
            {
        ...
        private void Buscar()
        {
            try
            {
                ...
                var list = new ApplicationDbContext().AjusteInventarios.Where(o =>
