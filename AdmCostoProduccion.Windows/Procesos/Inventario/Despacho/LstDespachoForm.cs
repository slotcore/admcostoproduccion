...
{
    public partial class LstDespachoForm : Form
    {
        private ObservableListSource<DespachoViewModel> ViewModelList = new ObservableListSource<DespachoViewModel>();

        #region Constructor
        public LstDespachoForm()
        {
            InitializeComponent();

            ViewModelList = new ObservableListSource<DespachoViewModel>();
            despachoViewModelBindingSource.DataSource = ViewModelList;
        }
        #endregion
        ...
        private void Agregar()
        {
            try
            {
                var frm = new MntDespachoForm(ViewModelList);
                frm.ShowDialog();
            }
            catch (Exception ex)
            {
        ...
        private void Modificar()
        {
            try
            {
                var viewModel = (DespachoViewModel)despachoViewModelBindingSource.Current;
                var frm = new MntDespachoForm(viewModel, ViewModelList);
                frm.ShowDialog();
            }
            catch (Exception ex)
            {
        ...
        private void Eliminar()
        {
            try
            {
                if (MessageBox.Show("¿Esta seguro de eliminar el registro?", "Eliminar", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    var viewModel = (DespachoViewModel)despachoViewModelBindingSource.Current;
                    var model = context.Despachos
                        .Where(o => o.DespachoId == viewModel.DespachoId)
                        .FirstOrDefault();
                    context.Despachos.Remove(model);
                    context.SaveChanges();
        ...
        private void Actualizar()
        {
            ...
            {
                Cursor = Cursors.WaitCursor;
                var list = new ApplicationDbContext().Despachos.ToList();
                ViewModelList.Clear();
                foreach (var model in list)
                {
                    ViewModelList.Add(new DespachoViewModel(model));
                }
            }
            catch (Exception ex)
            {
        ...
        private void Buscar()
        {
            try
            {
                ...
                var list = new ApplicationDbContext().Despachos.Where(o =>
                                o.Codigo.ToUpper().Contains(cadenaBusqueda) ||
                                o.Observacion.ToUpper().Contains(cadenaBusqueda) ||
                                o.Almacen.Nombre.ToUpper().Contains(cadenaBusqueda)).ToList();
