...
{
    public partial class LstVentaForm : Form
    {
        ...
        private void Agregar()
        {
            try
            {
                var frm = new MntVentaForm(ViewModelList);
                frm.ShowDialog();
            }
            catch (Exception ex)
            {
        ...
        private void Modificar()
        {
            try
            {
                var viewModel = (VentaViewModel)ventaViewModelBindingSource.Current;
                var frm = new MntVentaForm(viewModel, ViewModelList);
                frm.ShowDialog();
            }
            catch (Exception ex)
            {
        ...
        private void Eliminar()
        {
            try
            {
                if (MessageBox.Show("¿Esta seguro de eliminar el registro?", "Eliminar", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    var viewModel = (VentaViewModel)ventaViewModelBindingSource.Current;
                    var model = context.Ventas
                        .Where(o => o.VentaId == viewModel.VentaId)
                        .FirstOrDefault();
                    context.Ventas.Remove(model);
                    context.SaveChanges();
        ...
        private void Actualizar()
        {
            ...
            {
                Cursor = Cursors.WaitCursor;
                var list = new ApplicationDbContext().Ventas.ToList();
                ViewModelList.Clear();
