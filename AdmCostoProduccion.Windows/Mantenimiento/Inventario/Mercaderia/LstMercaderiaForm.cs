...
namespace AdmCostoProduccion.Windows.Mantenimiento.Inventario.Mercaderia
{
    public partial class LstMercaderiaForm : Form
    {
        ...
        private void Agregar()
        {
            try
            {
                var frm = new MntMercaderiaForm(ViewModelList);
                frm.ShowDialog();
            }
            catch (Exception ex)
            {
        ...
        private void Modificar()
        {
            try
            {
                var viewModel = (MercaderiaViewModel)mercaderiaViewModelBindingSource.Current;
                var frm = new MntMercaderiaForm(viewModel, ViewModelList);
                frm.ShowDialog();
            }
            catch (Exception ex)
            {
        ...
        private void Eliminar()
        {
            try
            {
                if (MessageBox.Show("¿Esta seguro de eliminar el registro?", "Eliminar", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    var viewModel = (MercaderiaViewModel)mercaderiaViewModelBindingSource.Current;
                    var model = context.Mercaderias
                        .Where(o => o.MercaderiaId == viewModel.MercaderiaId)
                        .FirstOrDefault();
                    context.Mercaderias.Remove(model);
                    context.SaveChanges();
        ...
        private void Actualizar()
        {
            ...
            {
                Cursor = Cursors.WaitCursor;
                var list = new ApplicationDbContext().Mercaderias.ToList();
                ViewModelList.Clear();
