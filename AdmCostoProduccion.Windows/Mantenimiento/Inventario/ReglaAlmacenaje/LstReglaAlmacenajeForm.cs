...
{
    public partial class LstReglaAlmacenajeForm : Form
    {
        private ObservableListSource<ReglaAlmacenajeViewModel> ViewModelList = new ObservableListSource<ReglaAlmacenajeViewModel>();

        #region Constructor
        public LstReglaAlmacenajeForm()
        {
            InitializeComponent();

            ViewModelList = new ObservableListSource<ReglaAlmacenajeViewModel>();
            reglaAlmacenajeViewModelBindingSource.DataSource = ViewModelList;
        }
        #endregion
        ...
        private void Agregar()
        {
            try
            {
                var frm = new MntReglaAlmacenajeForm(ViewModelList);
                frm.ShowDialog();
            }
            catch (Exception ex)
            {
        ...
        private void Modificar()
        {
            try
            {
                var viewModel = (ReglaAlmacenajeViewModel)reglaAlmacenajeViewModelBindingSource.Current;
                var frm = new MntReglaAlmacenajeForm(viewModel, ViewModelList);
                frm.ShowDialog();
            }
            catch (Exception ex)
            {
        ...
        private void Actualizar()
        {
            ...
            {
                Cursor = Cursors.WaitCursor;
                var list = new ApplicationDbContext().ReglaAlmacenajes.ToList();
                ViewModelList.Clear();
                foreach (var model in list)
                {
                    ViewModelList.Add(new ReglaAlmacenajeViewModel(model));
                }
            }
            catch (Exception ex)
            {
        ...
        private void Buscar()
        {
            try
            {
                ...
                var list = new ApplicationDbContext().ReglaAlmacenajes.Where(o =>
