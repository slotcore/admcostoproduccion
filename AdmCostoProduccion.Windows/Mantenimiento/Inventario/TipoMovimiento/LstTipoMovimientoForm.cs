...
{
    public partial class LstTipoMovimientoForm : Form
    {
        private ObservableListSource<TipoMovimientoViewModel> ViewModelList = new ObservableListSource<TipoMovimientoViewModel>();

        #region Constructor
        public LstTipoMovimientoForm()
        {
            InitializeComponent();

            ViewModelList = new ObservableListSource<TipoMovimientoViewModel>();
            tipoMovimientoViewModelBindingSource.DataSource = ViewModelList;
        }
        #endregion
        ...
        private void Agregar()
        {
            try
            {
                var frm = new MntTipoMovimientoForm(ViewModelList);
                frm.ShowDialog();
            }
            catch (Exception ex)
            {
        ...
        private void Modificar()
        {
            try
            {
                var viewModel = (TipoMovimientoViewModel)tipoMovimientoViewModelBindingSource.Current;
                var frm = new MntTipoMovimientoForm(viewModel, ViewModelList);
                frm.ShowDialog();
            }
            catch (Exception ex)
            {
        ...
        private void Eliminar()
        {
            try
            {
                if (MessageBox.Show("¿Esta seguro de eliminar el registro?", "Eliminar", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    var viewModel = (TipoMovimientoViewModel)tipoMovimientoViewModelBindingSource.Current;
                    var model = context.TipoMovimientos
                        .Where(o => o.TipoMovimientoId == viewModel.TipoMovimientoId)
                        .FirstOrDefault();
                    context.TipoMovimientos.Remove(model);
                    context.SaveChanges();
        ...
        private void Actualizar()
        {
            ...
            {
                Cursor = Cursors.WaitCursor;
                var list = new ApplicationDbContext().TipoMovimientos.ToList();
                ViewModelList.Clear();
                foreach (var model in list)
                {
                    ViewModelList.Add(new TipoMovimientoViewModel(model));
                }
            }
            catch (Exception ex)
            {
        ...
        private void Buscar()
        {
            try
            {
                ...
                var list = new ApplicationDbContext().TipoMovimientos.Where(o =>
