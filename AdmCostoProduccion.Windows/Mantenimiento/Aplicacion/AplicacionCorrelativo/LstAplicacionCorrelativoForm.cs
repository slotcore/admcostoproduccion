...
{
    public partial class LstAplicacionCorrelativoForm : Form
    {
        private ObservableListSource<AplicacionCorrelativoViewModel> ViewModelList = new ObservableListSource<AplicacionCorrelativoViewModel>();
        ...
        private void Agregar()
        {
            try
            {
                var frm = new MntAplicacionCorrelativoForm(ViewModelList);
                frm.ShowDialog();
            }
            catch (Exception ex)
            {
        ...
        private void Modificar()
        {
            try
            {
                var viewModel = (AplicacionCorrelativoViewModel)aplicacionCorrelativoViewModelBindingSource.Current;
                var frm = new MntAplicacionCorrelativoForm(viewModel, ViewModelList);
                frm.ShowDialog();
            }
            catch (Exception ex)
            {
        ...
        private void Actualizar()
        {
            ...
            {
                Cursor = Cursors.WaitCursor;
                var list = new ApplicationDbContext().AplicacionCorrelativos.ToList();
                ViewModelList.Clear();
                foreach (var model in list)
                {
                    ViewModelList.Add(new AplicacionCorrelativoViewModel(model));
                }
            }
            catch (Exception ex)
            {
        ...
        private void Buscar()
        {
            try
            {
                ...
                var list = new ApplicationDbContext().AplicacionCorrelativos.Where(o =>
                                o.Codigo.ToUpper().Contains(cadenaBusqueda) ||
                                o.Nombre.ToUpper().Contains(cadenaBusqueda) ||
                                o.Prefijo.ToUpper().Contains(cadenaBusqueda)).ToList();
