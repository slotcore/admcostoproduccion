...
{
    public partial class LstDistribucionValorizacionForm : Form
    {
        private ObservableListSource<DistribucionValorizacionViewModel> ViewModelList = new ObservableListSource<DistribucionValorizacionViewModel>();

        #region Constructor
        public LstDistribucionValorizacionForm()
        {
            InitializeComponent();

            ViewModelList = new ObservableListSource<DistribucionValorizacionViewModel>();
            distribucionValorizacionViewModelBindingSource.DataSource = ViewModelList;
        }
        #endregion
        ...
        private void Agregar()
        {
            try
            {
                var frm = new MntDistribucionValorizacionForm(ViewModelList);
                frm.ShowDialog();
            }
            catch (Exception ex)
            {
        ...
        private void Modificar()
        {
            try
            {
                var viewModel = (DistribucionValorizacionViewModel)distribucionValorizacionViewModelBindingSource.Current;
                var frm = new MntDistribucionValorizacionForm(viewModel, ViewModelList);
                frm.ShowDialog();
            }
            catch (Exception ex)
            {
        ...
        private void Actualizar()
        {
            ...
            {
                Cursor = Cursors.WaitCursor;
                var list = new ApplicationDbContext().DistribucionValorizacions.ToList();
                ViewModelList.Clear();
                foreach (var model in list)
                {
                    ViewModelList.Add(new DistribucionValorizacionViewModel(model));
                }
            }
            catch (Exception ex)
            {
        ...
        private void Buscar()
        {
            try
            {
                ...
                var list = new ApplicationDbContext().DistribucionValorizacions.Where(o =>
