...
{
    public partial class LstConfiguracionValorizacionForm : Form
    {
        private ObservableListSource<ConfiguracionValorizacionViewModel> ViewModelList = new ObservableListSource<ConfiguracionValorizacionViewModel>();
        ...
        private void Agregar()
        {
            try
            {
                var frm = new MntConfiguracionValorizacionForm(ViewModelList);
                frm.ShowDialog();
            }
            catch (Exception ex)
            {
        ...
        private void Modificar()
        {
            try
            {
                var viewModel = (ConfiguracionValorizacionViewModel)configuracionValorizacionViewModelBindingSource.Current;
                var frm = new MntConfiguracionValorizacionForm(viewModel, ViewModelList);
                frm.ShowDialog();
            }
            catch (Exception ex)
            {
        ...
        private void Actualizar()
        {
            ...
            {
                Cursor = Cursors.WaitCursor;
                var list = new ApplicationDbContext().ConfiguracionValorizacions.ToList();
                ViewModelList.Clear();
                foreach (var model in list)
                {
                    ViewModelList.Add(new ConfiguracionValorizacionViewModel(model));
                }
            }
            catch (Exception ex)
            {
        ...
        private void Buscar()
        {
            try
            {
                ...
                var list = new ApplicationDbContext().ConfiguracionValorizacions.Where(o =>
