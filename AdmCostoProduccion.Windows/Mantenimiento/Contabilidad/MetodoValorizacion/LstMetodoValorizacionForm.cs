...
{
    public partial class LstMetodoValorizacionForm : Form
    {
        private ObservableListSource<MetodoValorizacionViewModel> ViewModelList = new ObservableListSource<MetodoValorizacionViewModel>();
        ...
        private void Agregar()
        {
            try
            {
                var frm = new MntMetodoValorizacionForm(ViewModelList);
                frm.ShowDialog();
            }
            catch (Exception ex)
            {
        ...
        private void Modificar()
        {
            try
            {
                var viewModel = (MetodoValorizacionViewModel)metodoValorizacionViewModelBindingSource.Current;
                var frm = new MntMetodoValorizacionForm(viewModel, ViewModelList);
                frm.ShowDialog();
            }
            catch (Exception ex)
            {
        ...
        private void Actualizar()
        {
            ...
            {
                Cursor = Cursors.WaitCursor;
                var list = new ApplicationDbContext().MetodoValorizacions.ToList();
                ViewModelList.Clear();
                foreach (var model in list)
                {
                    ViewModelList.Add(new MetodoValorizacionViewModel(model));
                }
            }
            catch (Exception ex)
            {
        ...
        private void Buscar()
        {
            try
            {
                ...
                var list = new ApplicationDbContext().MetodoValorizacions.Where(o =>
